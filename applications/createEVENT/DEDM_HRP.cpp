...
{
  // DEDM_HRP -filenameBIM file -filenameEVENT file? <-getRV>
  ...
  if (argc == 6) { 
    ...
    json_t *generalInfo = json_object_get(root, "GeneralInformation");
    if (generalInfo == NULL) {
      return;
    }
	...
	json_array_append(newEventArray, value); // copy event for next application

      } else {

	json_t *eventObj = json_object();
	json_object_set(eventObj,"type", json_string("Wind"));
	json_object_set(eventObj,"subtype", json_string("FloorLoading"));

	// from general information
	json_t *width = json_object_get(generalInfo,"width");
	json_t *depth = json_object_get(generalInfo,"depth");
	json_t *height = json_object_get(generalInfo,"height");
	json_t *weight = json_object_get(generalInfo,"weight");

	json_t *modelPlan = json_object_get(value,"checkedPlan");
	json_t *modelHeight = json_object_get(value,"checkedHeight");
	json_t *modelExposure = json_object_get(value,"checkedExposure");
	json_t *windSpeed = json_object_get(value,"windSpeed");
