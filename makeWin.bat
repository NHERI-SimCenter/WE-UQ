...
:: makeWin.bat -- Build WE-UQ for Windows using MSVC + Ninja + Conan
::
...
::   - VTK 9.6.2 built into DEPS\ (run makeWinDeps.bat, or let this script do it)
...
:: ---- Build VTK + other DEPS if needed ----
...
copy /b WorkflowAppWE.cpp +,, >nul 2>&1
...
for %%f in ("%PREFIX%\bin\*.dll") do copy "%%f" "%APP_DIR%\" >nul
...
"%PYTHON%" -m pip install nheri-simcenter[weuq] --upgrade
